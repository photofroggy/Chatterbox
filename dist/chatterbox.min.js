...
};Chatterbox.UI.prototype.clock=function(a){if(a===undefined||a==this.settings.clock){return this.settings.clock}this.settings.clock=a;this.chatbook.retime();return this.settings.clock};Chatterbox.UI.prototype.get_user_media=function(b,c,a){Chatterbox._gum(b,c,a)};Chatterbox.UI.prototype.build=function(g,a,b){this.view.append(Chatterbox.render("ui",this.settings));this.control=new (g||Chatterbox.Control)(this);this.nav=new (a||Chatterbox.Navigation)(this);this.chatbook=new (b||Chatterbox.Chatbook)(this);this.pager=new Chatterbox.Pager(this);this.monitoro=this.chatbook.create_channel(this.mns,this.settings.monitor[1],true);this.monitoro.show();this.control.focus();this.sound.bank.m=this.view.find("div.soundbank");this.sound.add("click",this.sound.bank.m.find("audio.click")[0]);var f=this.sound;var d=false;var c=this.nav.add_button({label:"",icon:"volume",href:"#mute",title:"Mute the client",handler:function(){if(!d){f.mute();c.removeClass("volume");c.addClass("volume_mute");c.prop("title","Unmute the client");
d=true;return false}f.unmute();c.removeClass("volume_mute");c.addClass("volume");c.prop("title","Mute the client");d=false;return false}});this.ext.defaults=Chatterbox.Extension(this,this.client);var e=this;$(window).focus(function(){e.viewing=true});$(window).blur(function(){e.viewing=false});this.client.bind("pkt",function(j,h){e.packet(j,h)});this.client.bind("log",function(j,h){e.packet(j,h)});this.client.bind("pkt.whois",function(j,h){e.chatbook.current.log_whois(j.info)});this.client.middle("ns.remove",function(j,h){e.remove_channel(j.ns);h(j)});this.client.bind("ns.create",function(j,h){e.create_channel(j.chan.raw,j.chan.hidden)});this.client.bind("ns.user.list",function(h){e.channel(h.ns).set_user_list(h.users)});this.client.middle("ns.user.privchg",function(j,h){e.channel(j.ns).privchg(j,h)});this.client.bind("ns.user.remove",function(j,h){e.channel(j.ns).remove_one_user(j.user)});this.client.bind("ns.user.registered",function(h){e.channel(h.ns).register_user(h.user)})};Chatterbox.UI.prototype.resize=function(){this.control.resize();
this.view.height(this.view.parent().height());this.nav.resize();this.chatbook.resize(((this.view.parent().height()-this.nav.height())-this.control.height())-5)};Chatterbox.UI.prototype.loop=function(){this.chatbook.loop()};Chatterbox.UI.prototype.packet=function(b,a){var c=this;var d=this.protocol.log(b);if(d){if(this.settings.developer){console.log(">>>",b.sns,"|",d.text())}if(b.name=="log"&&b.sns=="~current"){b.ns=c.chatbook.current.raw;b.sns=c.chatbook.current.namespace}if(b.hasOwnProperty("s")&&b.s=="0"){this.chatbook.handle(b,a);return}b.html=d.html();this.cascade("log_message",function(f,e){c.chatbook.log_message(f.message,f.event)},{message:d,event:b})}this.chatbook.handle(b,a)};Chatterbox.UI.prototype.create_channel=function(b,a){this.chatbook.create_channel(b,a)};Chatterbox.UI.prototype.remove_channel=function(a){this.chatbook.remove_channel(a)};Chatterbox.UI.prototype.toggle_channel=function(a){return this.chatbook.toggle_channel(a)};Chatterbox.UI.prototype.channel=function(a,b){return this.chatbook.channel(a,b)
...
Chatterbox.template.settings.item.form.field.colour={};Chatterbox.template.settings.item.form.field.colour.post=Chatterbox.template.clean(["ref","default"]);Chatterbox.template.settings.item.form.field.colour.frame='<input class="{ref}" type="color" value="{default}" />';Chatterbox.Extension=function(c,a){var e=a.storage.folder("ui");var b={page:{},cmd:{}};c.storage=e;var d=function(){c.on("settings.open",b.page.settings);c.on("settings.open.ran",b.page.about);c.on("settings.save.ui",b.save);c.on("settings.save",function(){a.config_save()});a.bind("cmd.gettitle",b.cmd.gett);a.bind("cmd.gettopic",b.cmd.gett);a.bind("cmd.clear",b.cmd.clear);a.bind("cmd.clearall",b.cmd.clearall);a.bind("cmd.theme",b.cmd.theme);Chatterbox.Extension.Away(c,a,b);Chatterbox.Extension.Autojoin(c,a,b);Chatterbox.Extension.Ignore(c,a,b);b.load();b.save()};b.save=function(){e.set("theme",c.settings.theme);e.set("clock",c.settings.clock.toString());e.set("tabclose",c.settings.tabclose.toString())};b.load=function(){c.settings.theme=e.get("theme",c.settings.theme);
c.settings.clock=(e.get("clock",c.settings.clock.toString())=="true");c.settings.tabclose=(e.get("tabclose",c.settings.tabclose.toString())=="true")};b.page.settings=function(j,h){var g=j.settings.page("Main");var k={};k.username=a.settings.username;k.pk=a.settings.pk;k.devel=a.settings.developer;k.theme=replaceAll(h.settings.theme,"wsct_","");k.clock=h.clock();k.tc=h.nav.closer();var f=[];for(i in h.settings.themes){name=replaceAll(h.settings.themes[i],"wsct_","");f.push({value:name,title:name,selected:k.theme==name})}g.item("Text",{ref:"intro",title:"Main",text:"Use this window to view and change your settings.\n\nCheck                    the different pages to see what settings can be changed.",});g.item("Form",{ref:"login",title:"Login",text:"Here you can change the username and token used to                    log into the chat server.",fields:[["Textfield",{ref:"username",label:"Username","default":k.username}],["Textfield",{ref:"token",label:"Token","default":k.pk}]],event:{save:function(l){a.settings.username=l.data.username;
a.settings.pk=l.data.token}}});g.item("Form",{ref:"ui",title:"UI",hint:"<b>Timestamp</b><br/>Choose between a 24 hour clock and                    a 12 hour clock.\n\n<b>Theme</b><br/>Change the look of the                    client.\n\n<b>Close Buttons</b><br/>Turn tab close buttons on/off.",fields:[["Dropdown",{ref:"theme",label:"Theme",items:f}],["Dropdown",{ref:"clock",label:"Timestamp Format",items:[{value:"24",title:"24 hour",selected:k.clock},{value:"12",title:"12 hour",selected:!k.clock}]}],["Checkbox",{ref:"tabclose",label:"Close Buttons",items:[{value:"yes",title:"On",selected:k.tc}]}],],event:{change:function(l){h.clock(l.data.clock=="24");h.theme(l.data.theme);h.nav.closer(l.data.tabclose.indexOf("yes")>-1)},save:function(l){k.clock=h.clock();k.theme=replaceAll(h.theme(),"wsct_","");k.tc=h.nav.closer();h.trigger("settings.save.ui",{clock:k.clock,tabclose:k.tc,theme:"wsct_"+k.theme})},close:function(l){h.clock(k.clock);h.theme(k.theme);h.nav.closer(k.tc)}}});g.item("Form",{ref:"developer",title:"Developer Mode",text:"Turn developer mode on or off.\n\nDeveloper mode will expose any hidden                channel tabs, amongst other things. Keep this turned off unless you're working                on implementing something.",fields:[["Checkbox",{ref:"enabled",items:[{value:"on",title:"On",selected:k.devel}]}]],event:{change:function(l){a.settings.developer=(l.data.enabled.indexOf("on")!=-1);
h.developer(a.settings.developer)},save:function(l){k.devel=a.settings.developer},close:function(l){a.settings.developer=k.devel;h.developer(a.settings.developer)}}});g.item("Text",{ref:"debug",wclass:"faint",title:"Debug Information",text:"Chat Agent: <code>"+a.settings.agent+"</code>\n\nUser                    Agent: <code>"+navigator.userAgent+"</code>"})};b.page.about=function(h,g){var f=h.settings.page("About",true);f.item("Text",{ref:"about-wsc",title:"Wsc",text:'Currently using <a href="http://github.com/photofroggy/wsc/">wsc</a>                    version '+wsc.VERSION+" "+wsc.STATE+'.\n\nWsc                    works using HTML5, javascript, and CSS3. WebSocket is used for the connection                    where possible. The source code for this client is pretty huge.\n\nWsc was created                    by ~<a href="http://photofroggy.deviantart.com/">photofroggy</a>'})};b.cmd.gett=function(g,f){var h=g.cmd.indexOf("title")>-1?"title":"topic";c.control.set_text("/"+h+" "+f.channel(g.target).info[h].content)
};b.cmd.theme=function(g,f){c.theme(g.args.split(" ").shift())};b.cmd.clear=function(h,f){if(h.args.length>0){var j=h.args.split(" ");for(var g in j){if(!j.hasOwnProperty(g)){continue}c.channel(h.target).clear_user(j[g])}}else{c.channel(h.target).clear()}};b.cmd.clearall=function(g,f){var j=null;if(g.args.length>0){var h=g.args.split(" ");j=function(l,m){for(var k in h){if(!h.hasOwnProperty(k)){continue}m.clear_user(h[k])}}}else{j=function(k,l){l.clear()}}c.chatbook.each(j,true)};d();return b};Chatterbox.Extension.Away=function(d,a,c){c.away={};var b=a.ext.defaults.away;var e=function(){d.on("settings.open",c.away.page);d.on("settings.save",b.save);a.bind("ext.away.away",c.away.away);a.bind("ext.away.back",c.away.back)};c.away.page=function(g,j){var f=function(m){m=replaceAll(m,"<","&lt;");m=replaceAll(m,">","&gt;");m=replaceAll(m,'"',"&quot;");return m};var k=function(m){m=replaceAll(m,"&lt;","<");m=replaceAll(m,"&gt;",">");m=replaceAll(m,"&quot;",'"');return m};var h=g.settings.page("Away");
var l={};l.away=b.format.away;l.sa=b.format.setaway;l.sb=b.format.setback;l.intr=b.interval;h.item("Text",{ref:"intro",title:"Away Messages",text:"Use away messages when you are away from the chats.\n\n                    You can set yourself away using the                     <code>/setaway [reason]</code> command. When you get back,                    use <code>/setback</code>. While you are away, the client                    will automatically respond when people try to talk to you,                    telling them you're away.",});h.item("Form",{ref:"msgs",title:"Messages",text:"Here you can set the messages displayed when you set                    yourself away or back. You can also change the away message                    format.",hint:"<b>{user}</b><br/>This is replaced with the username of the person trying to talk to you\n\n<b>{reason}</b>                    <br/>This is replaced by your reason for being away.\n\n<b>{timesince}</b>                    <br/>Use this to show how long you have been away for.",fields:[["Textfield",{ref:"away",label:"Away","default":f(l.away)}],["Textfield",{ref:"setaway",label:"Setaway","default":f(l.sa)}],["Textfield",{ref:"setback",label:"Setback","default":f(l.sb)}]],event:{save:function(m){b.format.away=k(m.data.away);
b.format.setaway=k(m.data.setaway);b.format.setback=k(m.data.setback);b.save()}}});h.item("Form",{ref:"interval",title:"Message Interval",text:"Here you can set the amount of time to wait before                     displaying another away message. The interval is in seconds.",fields:[["Textfield",{ref:"interval",label:"Interval","default":(l.intr/1000).toString()}]],event:{save:function(m){b.interval=parseInt(m.data.interval)*1000;b.save()}}})};c.away.away=function(f){d.control.add_state({ref:"away",label:"Away, reason: <i>"+f.reason+"</i>"})};c.away.back=function(f){d.control.rem_state("away")};e()};Chatterbox.Extension.Autojoin=function(d,a,c){var b=a.ext.defaults.autojoin;c.autojoin={};var e=function(){d.on("settings.open",c.autojoin.page);d.on("settings.save",b.save)};d.nav.add_button({icon:"chat",label:"",title:"Join your autojoin channels",href:"#autojoin-do",handler:b.join});c.autojoin.page=function(j,l){var k=j.settings.page("Autojoin");var h="<ul>";var m={};m.ajon=b.on;m.chan=b.channel;
if(b.channel.length==0){h+="<li><i>No autojoin channels set</i></li></ul>"}else{for(var g in b.channel){if(!b.channel.hasOwnProperty(g)){continue}h+="<li>"+b.channel[g]+"</li>"}h+="</ul>"}k.item("Checkbox",{ref:"eaj",title:"Autojoin",text:"Turn on autojoin to automatically join selected channels                    when you connect to the chat server.",items:[{value:"yes",title:"On",selected:m.ajon}],event:{change:function(n){if(n.target.value=="yes"){b.on=n.target.checked}},save:function(n){m.ajon=b.on;a.config_save()},close:function(n){b.on=m.ajon}}});var f=k.item("Items",{ref:"channelss",title:"Channels",text:"Add any channels you want to join automatically when you                    connect to the chat server.",items:b.channel,prompt:{title:"Add Channel",label:"Channel:",},event:{up:function(n){var o=n.args.swap;b.channel[o["this"].index]=o.that.item;b.channel[o.that.index]=o["this"].item;f.options.items=b.channel},down:function(n){var o=n.args.swap;b.channel[o["this"].index]=o.that.item;
b.channel[o.that.index]=o["this"].item;f.options.items=b.channel},add:function(p){var o=a.deform_ns(p.args.item).toLowerCase();var n=b.channel.indexOf(o);if(n!=-1){return}b.channel.push(o);f.options.items=b.channel},remove:function(n){b.channel.splice(n.args.index,1);f.options.items=b.channel},save:function(n){m.chan=b.channel;a.config_save()},close:function(n){a.config_load()}}})};e()};Chatterbox.Extension.Ignore=function(d,a,c){var b=a.ext.defaults.ignore;c.ignore={};var e=function(){};var f=function(){d.on("settings.open",c.ignore.page);a.bind("ignore.loaded",c.ignore.loaded);a.bind("ignore.add",c.ignore.add);a.bind("ignore.remove",c.ignore.remove)};c.ignore.loaded=function(h){for(var g=0;g<h.ignored.length;g++){if(!h.ignored.hasOwnProperty(g)){continue}d.mute_user(h.ignored[g])}};c.ignore.add=function(g){d.mute_user(g.user);if(g.suppress){return}var h=replaceAll(b.fignore,"{user}",g.user);if(h.indexOf("/me")==0){h=h.substr(4);a.action(d.chatbook.current.namespace,h);return}a.say(d.chatbook.current.namespace,h)
};c.ignore.remove=function(g){d.unmute_user(g.user);if(g.suppress){return}var h=replaceAll(b.funignore,"{user}",g.user);if(h.indexOf("/me")==0){h=h.substr(4);a.action(d.chatbook.current.namespace,h);return}a.say(d.chatbook.current.namespace,h)};c.ignore.page=function(h,k){var j=h.settings.page("Ignores");var l={};l.im=b.fignore;l.uim=b.funignore;l.usr=b.ignored;j.item("Text",{ref:"intro",title:"Ignores",text:'Use <code>ignore</code> to ignore people.\n\n                    You can "ignore" other users of the chat server using the\n                    <code>/ignore</code> command. Ignoring a user hides their                    messages from you in the channel log.',});j.item("Form",{ref:"msgs",title:"Messages",text:"Here you can set the messages displayed when you ignore or                    unignore a user.\n\nThe text <code>{user}</code> is replaced                    with the name of the user your are ignoring or unignoring.",fields:[["Textfield",{ref:"ignore",label:"Ignore","default":l.im}],["Textfield",{ref:"unignore",label:"Unignore","default":l.uim}]],event:{save:function(m){b.fignore=m.data.ignore;
b.funignore=m.data.unignore;b.save()}}});var g=j.item("Items",{ref:"ignoreds",title:"Users",text:"This is the list of users that you have silenced.\n\nUse the                    commands <code>/ignore</code> and <code>/unignore</code>                    to edit the list.",items:l.usr,prompt:{title:"Add User",label:"User:",},event:{up:function(m){var n=m.args.swap;b.ignored[n["this"].index]=n.that.item;b.ignored[n.that.index]=n["this"].item;g.options.items=b.ignored},down:function(m){var n=m.args.swap;b.ignored[n["this"].index]=n.that.item;b.ignored[n.that.index]=n["this"].item;g.options.items=b.ignored},add:function(m){b.add(m.args.item,true);g.options.items=b.ignored},remove:function(m){b.remove(m.args.item,true);g.options.items=b.ignored},save:function(m){l.usr=b.ignored;b.save()},close:function(m){b.load()}}})};f()};