...
Chatterbox.template.settings.item.form.field.colour={};Chatterbox.template.settings.item.form.field.colour.post=Chatterbox.template.clean(["ref","default"]);Chatterbox.template.settings.item.form.field.colour.frame='<input class="{ref}" type="color" value="{default}" />';Chatterbox.Extension=function(c,a){var e=a.storage.folder("ui");var b={page:{}};c.storage=e;var d=function(){c.on("settings.open",b.page.settings);c.on("settings.open.ran",b.page.about);c.on("settings.save.ui",b.save);c.on("settings.save",function(){a.config_save()});Chatterbox.Extension.Away(c,a,b);b.load();b.save()};b.save=function(){e.set("theme",c.settings.theme);e.set("clock",c.settings.clock.toString());e.set("tabclose",c.settings.tabclose.toString())};b.load=function(){c.settings.theme=e.get("theme",c.settings.theme);c.settings.clock=(e.get("clock",c.settings.clock.toString())=="true");c.settings.tabclose=(e.get("tabclose",c.settings.tabclose.toString())=="true")};b.page.settings=function(j,h){console.log(">> main settings");
var g=j.settings.page("Main");var k={};k.username=a.settings.username;k.pk=a.settings.pk;k.devel=a.settings.developer;k.theme=replaceAll(h.settings.theme,"wsct_","");k.clock=h.clock();k.tc=h.nav.closer();var f=[];for(i in h.settings.themes){name=replaceAll(h.settings.themes[i],"wsct_","");f.push({value:name,title:name,selected:k.theme==name})}g.item("Text",{ref:"intro",title:"Main",text:"Use this window to view and change your settings.\n\nCheck                    the different pages to see what settings can be changed.",});g.item("Form",{ref:"login",title:"Login",text:"Here you can change the username and token used to                    log into the chat server.",fields:[["Textfield",{ref:"username",label:"Username","default":k.username}],["Textfield",{ref:"token",label:"Token","default":k.pk}]],event:{save:function(l){a.settings.username=l.data.username;a.settings.pk=l.data.token}}});g.item("Form",{ref:"ui",title:"UI",hint:"<b>Timestamp</b><br/>Choose between a 24 hour clock and                    a 12 hour clock.\n\n<b>Theme</b><br/>Change the look of the                    client.\n\n<b>Close Buttons</b><br/>Turn tab close buttons on/off.",fields:[["Dropdown",{ref:"theme",label:"Theme",items:f}],["Dropdown",{ref:"clock",label:"Timestamp Format",items:[{value:"24",title:"24 hour",selected:k.clock},{value:"12",title:"12 hour",selected:!k.clock}]}],["Checkbox",{ref:"tabclose",label:"Close Buttons",items:[{value:"yes",title:"On",selected:k.tc}]}],],event:{change:function(l){h.clock(l.data.clock=="24");
h.theme(l.data.theme);h.nav.closer(l.data.tabclose.indexOf("yes")>-1)},save:function(l){k.clock=h.clock();k.theme=replaceAll(h.theme(),"wsct_","");k.tc=h.nav.closer();h.trigger("settings.save.ui",{clock:k.clock,tabclose:k.tc,theme:"wsct_"+k.theme})},close:function(l){h.clock(k.clock);h.theme(k.theme);h.nav.closer(k.tc)}}});g.item("Form",{ref:"developer",title:"Developer Mode",text:"Turn developer mode on or off.\n\nDeveloper mode will expose any hidden                channel tabs, amongst other things. Keep this turned off unless you're working                on implementing something.",fields:[["Checkbox",{ref:"enabled",items:[{value:"on",title:"On",selected:k.devel}]}]],event:{change:function(l){a.settings.developer=(l.data.enabled.indexOf("on")!=-1);h.developer(a.settings.developer)},save:function(l){k.devel=a.settings.developer},close:function(l){a.settings.developer=k.devel;h.developer(a.settings.developer)}}});g.item("Text",{ref:"debug",wclass:"faint",title:"Debug Information",text:"Chat Agent: <code>"+a.settings.agent+"</code>\n\nUser                    Agent: <code>"+navigator.userAgent+"</code>"})
};b.page.about=function(h,g){var f=h.settings.page("About",true);f.item("Text",{ref:"about-wsc",title:"Wsc",text:'Currently using <a href="http://github.com/photofroggy/wsc/">wsc</a>                    version '+wsc.VERSION+" "+wsc.STATE+'.\n\nWsc                    works using HTML5, javascript, and CSS3. WebSocket is used for the connection                    where possible. The source code for this client is pretty huge.\n\nWsc was created                    by ~<a href="http://photofroggy.deviantart.com/">photofroggy</a>'})};d();return b};Chatterbox.Extension.Away=function(d,a,c){c.away={};var b=a.ext.defaults.away;var e=function(){d.on("settings.open",c.away.page);a.bind("ext.away.away",c.away.away);a.bind("ext.away.back",c.away.back)};c.away.page=function(g,j){var f=function(m){m=replaceAll(m,"<","&lt;");m=replaceAll(m,">","&gt;");m=replaceAll(m,'"',"&quot;");return m};var k=function(m){m=replaceAll(m,"&lt;","<");m=replaceAll(m,"&gt;",">");m=replaceAll(m,"&quot;",'"');return m};var h=g.settings.page("Away");
